    ...
    <section className="bg-[#EBF4FF]">
      <div className="max-w-[1280px] mx-auto px-6 py-12 md:px-10 md:py-16 lg:px-16 lg:py-20">
        <header ref={headingRef} className="flex items-start gap-3 md:gap-4">
          <div className="w-1.5 h-8 md:h-12 lg:h-14 flex-shrink-0 rounded-full bg-memoli-accent mt-1 md:mt-2" aria-hidden />
          <div>
            {/* Audit: Desktop 44px, Tablet 40px, Mobile 24px #3C7CF7 / #152B56 */}
            <h2 className="text-memoli-dark font-bold text-2xl md:text-[40px] lg:text-[44px] leading-tight">
              Vision of a Sustainable Future for{" "}
              <span className="text-memoli-accent">Nurturing Healthy Families</span>
            </h2>
            <p className="mt-2 text-memoli-dark font-medium text-base md:text-2xl lg:text-[32px] leading-relaxed">
        ...
        <div ref={cardsRef} className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 mt-8 md:mt-10 lg:mt-12">
          {visionCards.map((card, index) => (
            <article key={index} className="flex flex-col">
              <div className="relative w-full aspect-[4/3] overflow-hidden rounded-[24px]">
                <Image
                  src={card.image}
                  alt=""
                  fill
              ...
              <p className="mt-4 text-memoli-dark font-medium text-base md:text-2xl lg:text-[28px] leading-relaxed text-center">
                {card.description}
              </p>
            </article>
          ))}
